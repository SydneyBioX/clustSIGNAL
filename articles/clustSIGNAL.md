...
Other parameters that can be modified include: (i) dimRed_init - low
dimension data to use (default ‘None’) for initial clustering; (ii)
dimRed_f - low dimension data to use (default ‘None’) for final
clustering; (iii) batch - when TRUE, ClustSIGNAL performs batch
correction and needs a valid value for batch_by; (iv) batch_by - name of
metadata column containing sample batches contributing to batch effect
(default ‘None’); (v) NN - neighbourhood size (default 30); (vi)
kernel - distribution to use for weight generation (default ‘G’ for
Gaussian); (vii) spread - distribution spread value (default 0.3 for
Gaussian); (viii) sort - when TRUE, ClustSIGNAL sorts the neighbourhood;
(ix) threads - number of cpus to use in parallel runs (default 1); and
(x) clustParams - list of parameters to use for non-spatial clustering
components.
...
```
    ...
    ##                      Cells Clusters
    ## 1 embryo2_Pos29_cell100_z2        1
    ## 2 embryo2_Pos29_cell101_z5        1
    ## 3 embryo2_Pos29_cell104_z2        1
    ## 4 embryo2_Pos29_cell104_z5        1
    ## 5 embryo2_Pos29_cell105_z2        1
    ## 6 embryo2_Pos29_cell108_z5        1
...
box_clust <- df_met %>%
  ...
  labs(x = "ClustSIGNAL clusters", y = "Entropy", title = "Clusters") +
  theme_classic() +
...
spe2 <- SpatialExperiment(assays = list(logcounts = mh_expr), 
                          ...
                          spatialCoordsNames = c("X", "Y"))
spe2$Cell_class <- factor(spe2$Cell_class)
spe2
```
...
```
    ...
    ##   .. ..$ Cell_class: Factor w/ 15 levels "Astrocyte","Endothelial 1",..: 2 2 15 1 1 7 7 3 7 7 ...
...
```
    ...
    ## 1 1.-0.09 0.383 0.518       0.612                 0        4.20         3.19
    ## 2 7.-0.09 0.392 0.549       1.10                  0        4.41         3.16
    ## 3 7.0.16  0.568 0.603       0.883                 0        4.28         3.09
...
uses either 3000 centers or 1/5th of the total cells in the data as the
...
```
    ...
    ##                      7.2                      3.3                      7.2 
    ## 34 Levels: 1.1 1.2 1.3 2.1 2.2 2.3 2.4 2.5 3.1 3.2 3.3 3.4 4.1 4.2 4.3 ... 9.4
...
outReg$regXclust[1:3, 1:3]
```

    ##                          1.1 1.2 1.3
    ## embryo2_Pos29_cell100_z2   0   0   0
    ## embryo2_Pos29_cell101_z5   0   0   0
    ## embryo2_Pos29_cell104_z2   0   0   0
...
```
    ...
    ## embryo2_Pos29_cell100_z2 embryo2_Pos29_cell101_z5 embryo2_Pos29_cell104_z2 
    ##                1.0118920                0.2055925                0.6120693 
    ## embryo2_Pos29_cell104_z5 embryo2_Pos29_cell105_z2 embryo2_Pos29_cell108_z5 
    ##                0.5475532                0.2055925                0.2055925
...
```
    ...
    ## Abcc4                 0.32885385                0.2841961
    ## Acp5                  0.08959597                0.1189280
    ## Acvr1                 0.40086614                0.3359336
    ## Acvr2a                0.51895167                0.5066586
    ## Adora2b               0.12246319                0.1257018
